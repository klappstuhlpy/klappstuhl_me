...
impl Backend {
    ...
    pub async fn detect(override_kind: Option<&str>) -> Self {
        let elevation = if which("sudo").await { Some("sudo".to_string()) } else { None };

        if let Some(forced) = override_kind.and_then(BackendKind::parse) {
            return Backend { kind: forced, elevation };
        }
        ...
        Backend { kind: BackendKind::Disabled, elevation }
    }
        ...
        match self.kind {
            ...
            BackendKind::Nftables => {
                ...
                if add {
                    ...
                    Some(self.wrap(vec![
                        "nft", "flush", "rule", "inet", "filter", "input",
                        "ip", "saddr", ip,
                    ]))
                }
            }
...
async fn probe(kind: BackendKind) -> bool {
    let argv: &[&str] = match kind {
        BackendKind::Nftables => &["nft", "list", "tables"],
        BackendKind::Ufw => &["ufw", "status"],
        BackendKind::Iptables => &["iptables", "--version"],
