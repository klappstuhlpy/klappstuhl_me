    ...
    state
        ...
        .call(|conn| -> rusqlite::Result<Vec<FirewallRule>> {
            ...
            let rows: rusqlite::Result<Vec<_>> =
                stmt.query_map([], FirewallRule::from_row)?.collect();
            rows
        })
        .await
}
...
pub async fn list_lockouts(
    state: &AppState,
    only_active: bool,
) -> rusqlite::Result<Vec<LockoutRow>> {
    state
        .database()
        .call(move |conn| -> rusqlite::Result<Vec<LockoutRow>> {
            ...
            let rows: rusqlite::Result<Vec<_>> =
                stmt.query_map([], LockoutRow::from_row)?.collect();
            rows
        })
        .await
}

pub async fn find_active_lockout(
    state: &AppState,
    ip: &str,
) -> rusqlite::Result<Option<LockoutRow>> {
    let ip = ip.to_string();
    state
        .database()
    ...
    state
        ...
        .call(move |conn| -> rusqlite::Result<i64> {
            ...
            let existing: Option<i64> = conn
                .prepare_cached(
                    "SELECT id FROM firewall_lockout WHERE ip = ? AND status = 'active'",
                )?
...
pub async fn release_lockout(state: &AppState, id: i64) -> rusqlite::Result<usize> {
    state
        ...
        .call(move |conn| {
            conn.execute(
                "UPDATE firewall_lockout SET status = 'released' WHERE id = ?",
                [id],
            )
        })
        .await
