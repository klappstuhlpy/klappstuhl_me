//! Read-only Docker introspection layer.
//!
...
use bollard::{
    ...
    models::{
        ContainerInspectResponse, ContainerSummary, Network, PortTypeEnum, Volume,
    },
...
use crate::{cached::TimedCachedValue, AppState};
...
// ─── Graph types ─────────────────────────────────────────────────────────────
//
// These are the serialisable types the graph endpoint (Phase 2.2) returns.
// Defined here so the docker module owns the full data model.
                        ...
                        .filter_map(|p| {
                            ...
                            match (p.public_port, p.private_port) {
                                (Some(pub_port), priv_port) => {
                                    Some(format!("{}:{}/{}", pub_port, priv_port, proto))
                                }
                                (None, priv_port) => Some(format!("{}/{}", priv_port, proto)),
                            }
                        })
                        .collect()
                })
