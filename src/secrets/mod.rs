...
pub async fn run_scan(state: &AppState) -> anyhow::Result<()> {
    ...
    let (findings, counters) =
        tokio::task::spawn_blocking(move || scanner::scan(&roots)).await?;

    let total = findings.len();
    let critical = findings.iter().filter(|f| matches!(f.severity, rules::Severity::Critical)).count();

    let new_count = storage::record_scan(
        state,
        findings,
