...
#[derive(Template)]
#[template(path = "admin_firewall.html")]
...
async fn page(
    ...
    account: Account,
) -> Result<AdminFirewallTemplate, StatusCode> {
...
async fn reapply_all(
    ...
    for rule in rules {
        ...
        let Some(argv) = backend.apply_command(&rule) else { continue };
        let preview = Backend::render(&argv);
        match backend.exec(argv).await {
...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/admin/firewall/lockout/:id/release",
            post(release_lockout),
        )
