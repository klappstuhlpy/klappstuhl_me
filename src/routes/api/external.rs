...
fn unavailable(tool: &str) -> ApiError {
    ApiError::new(format!("{tool} is not available on this server"))
...
pub async fn screenshot(
    ...
    state.audit("api.render.screenshot").actor(&account).target(req.url).ip_opt(client_ip).fire();
...
pub async fn markdown_pdf(
    ...
    state.audit("api.render.markdown_pdf").actor(&account).ip_opt(client_ip).fire();
    Ok(shared_or_bytes(&state, pdf, "application/pdf", q.share.unwrap_or(false)))
}
...
pub async fn transcode(
    ...
    let (in_ext, out_ext, out_args, mime): (&str, &str, Vec<&str>, &str) = match to.as_str() {
        "mp4" => ("mov", "mp4", vec!["-c:v", "libx264", "-c:a", "aac", "-movflags", "+faststart"], "video/mp4"),
    ...
    state.audit("api.convert.transcode").actor(&account).target(to).ip_opt(client_ip).fire();
