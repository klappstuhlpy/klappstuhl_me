...
use crate::{headers::ClientIp, models::Account, postgres, AppState};
use askama::Template;
use axum::{
    extract::{Query, State},
    http::StatusCode,
...
#[derive(Template)]
#[template(path = "admin/admin_postgres.html")]
...
async fn postgres_page(State(state): State<AppState>, account: Account) -> Result<AdminPostgresTemplate, StatusCode> {
...
async fn list_databases(
    ...
    check_admin_and_pg(&state, &account).map_err(|s| {
        (
            s,
            Json(ApiError {
...
async fn run_query(
    ...
    if safe && !postgres::is_safe_query(&payload.sql) {
        ...
        return Err((
            ...
            Json(ApiError {
                error: "Blocked by safe-mode: only SELECT / EXPLAIN / SHOW / WITH / VALUES / TABLE / FETCH allowed."
                    .into(),
            }),
        ));
    }
