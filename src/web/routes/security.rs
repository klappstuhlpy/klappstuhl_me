...
#[derive(Template)]
#[template(path = "admin/admin_security.html")]
...
async fn security_page(State(state): State<AppState>, account: Account) -> Result<AdminSecurityTemplate, StatusCode> {
...
async fn cloudflare_data(
    ...
    let summary = cf.zone_summary(since).await.map_err(|_| StatusCode::BAD_GATEWAY)?;
