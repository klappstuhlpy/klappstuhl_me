...
async fn reconcile(state: &AppState, tasks: &TaskMap) -> anyhow::Result<()> {
    ...
    let active_ids: std::collections::HashSet<i64> = targets
        .iter()
        .filter(|t| t.enabled)
        .map(|t| t.id)
        .collect();
...
pub async fn run_check_now(state: &AppState, target_id: i64) -> anyhow::Result<CheckOutcome> {
    ...
    match (&outcome.status, prev_open) {
        ...
        (CheckStatus::Down | CheckStatus::Degraded, None) => {
            ...
            let consecutive =
                storage::consecutive_failures(state, target_id, INCIDENT_THRESHOLD).await?;
            if consecutive >= INCIDENT_THRESHOLD {
                ...
                storage::open_incident(state, target_id, status_label, outcome.error.as_deref())
                    .await?;
