...
pub async fn list_targets(state: &AppState) -> rusqlite::Result<Vec<HealthTarget>> {
    state
        ...
        .call(|conn| -> rusqlite::Result<Vec<HealthTarget>> {
            ...
            let rows: rusqlite::Result<Vec<HealthTarget>> =
                stmt.query_map([], HealthTarget::from_row)?.collect();
            rows
        })
        .await
}
...
pub async fn get_open_incident(
    state: &AppState,
    target_id: i64,
) -> rusqlite::Result<Option<IncidentRow>> {
    state
        .database()
...
pub async fn list_summaries(state: &AppState) -> rusqlite::Result<Vec<TargetSummary>> {
    ...
    for target in targets {
        ...
        let (last_status, last_latency, last_code, last_error_opt, last_ssl_days, last_check) = state
            .database()
            .call(move |conn| -> rusqlite::Result<(Option<String>, Option<i64>, Option<i64>, Option<String>, Option<i64>, Option<OffsetDateTime>)> {
                let mut stmt = conn.prepare_cached(
