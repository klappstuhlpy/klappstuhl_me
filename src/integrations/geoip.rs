...
//! configured by `geoip_db_path` (default `/data/geoip/GeoLite2-City.mmdb`)
//! and the security dashboard will start populating country / city / ASN.
//!
...
impl GeoIp {
    ...
    pub fn lookup(&self, ip: IpAddr) -> Option<GeoRecord> {
        ...
        if country_code.is_empty() && city_name.is_empty() && latitude.is_none() {
            return None;
        }
