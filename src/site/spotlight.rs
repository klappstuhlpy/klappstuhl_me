...
fn static_nav() -> Vec<SpotlightItem> {
    vec![
        ...
        SpotlightItem::nav("Logs", "Tail and filter application logs", "/admin/logs/view"),
    ]
}
...
fn static_pages() -> Vec<SpotlightItem> {
    vec![
        ...
        SpotlightItem::page("Changelog", "What's new, release by release", "/changelog"),
...
async fn search(State(state): State<AppState>, account: Account, Query(params): Query<SearchQuery>) -> Response {
...
async fn run_script(State(state): State<AppState>, account: Account, Json(payload): Json<RunPayload>) -> Response {
    if !account.flags.is_admin() {
        return StatusCode::FORBIDDEN.into_response();
    }
    ...
    let Some(script) = script else {
        return (
            ...
            Json(serde_json::json!({ "error": "unknown script id" })),
        )
            .into_response();
    };
    ...
    let output = match result {
        ...
        Err(_) => {
            return (
                ...
                Json(serde_json::json!({ "error": "script timed out (30s)" })),
            )
                .into_response();
        }
    };
