...
//!
//! Shells out to `docker stats --no-stream --format "{{json .}}"` which prints
//! one JSON object per running container.  Parsing JSON keeps us independent
//! of the column widths that the human-readable format uses.
...
impl DockerStat {
    pub fn mem_pct(&self) -> f64 {
        if self.mem_limit == 0 {
            ...
            self.mem_used as f64 / self.mem_limit as f64 * 100.0
        }
    }
}
