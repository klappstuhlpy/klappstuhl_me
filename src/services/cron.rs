...
/// Builds the shell command for a script string. On Unix the standard system
/// `bin` directories are appended to `PATH` so commands work even when the
/// process was launched (e.g. by systemd) with a stripped environment.
#[cfg(unix)]
        ...
        loop {
            ...
            for script in &state.config().spotlight_scripts {
                ...
                match CronSchedule::parse(expr) {
                    Ok(sched) if sched.matches(now) => run_scheduled(&state, script).await,
                    _ => {}
                }
            }
        }
...
/// Runs one scheduled script, logging the outcome and writing an audit entry.
async fn run_scheduled(state: &AppState, script: &SpotlightScript) {
    ...
    match outcome {
        Ok(Ok(output)) => {
            ...
            state
                .audit("spotlight.script.scheduled")
                .actor_label("scheduler")
                .target(script.name.clone())
                .meta(serde_json::json!({ "exit_code": code, "success": output.status.success() }))
                .fire();
        }
