...
//!
//! When `cloudflare_api_token` + `cloudflare_account_id` + `cloudflared_tunnel_id`
//! are configured, the cloudflared proxy backend manages the tunnel's
//! public-hostname ingress through the Cloudflare API instead of writing a
//! local `config.yml`:
//!
//! - **import** pulls the tunnel's current ingress into the `proxy_route` table
//!   (so dashboard-created routes show up in `/admin/proxy`);
//! - **push** writes every enabled route back as the tunnel's ingress and
//!   upserts the matching proxied `CNAME` DNS records.
//!
//! Cloudflare propagates the pushed config to the running `cloudflared`
//! automatically, so there's no reload command to run — which is exactly why
//! this works for a `cloudflared` container with no systemd.

use crate::cf_tunnel::{CfTunnel, IngressRule};
use crate::{proxy::storage, AppState};
...
/// Returns `None` for non-HTTP services (tcp/ssh/rdp/unix/http_status) we don't
/// model as proxy routes.
pub fn parse_service(service: &str) -> Option<(String, String, i64)> {
    ...
    let (host, port) = match authority.rsplit_once(':') {
        Some((h, p)) => match p.parse::<i64>() {
            ...
            // Colon but not a port (shouldn't happen for http) — treat as host.
...
/// Builds the ingress list the tunnel should serve from the enabled routes,
/// always terminated by the required `http_status:404` catch-all.
fn build_ingress(routes: &[storage::ProxyRoute]) -> Vec<IngressRule> {
    let mut rules: Vec<IngressRule> = routes
        ...
        .map(|r| {
            ...
            // Internal https origins are commonly self-signed.
...
/// proxied CNAME for each hostname. DNS failures are non-fatal (collected into
/// the report) so one bad hostname doesn't block the rest.
pub async fn push(state: &AppState) -> anyhow::Result<ApplyReport> {
    ...
    // Upsert DNS for each enabled hostname (best-effort).
...
/// Pulls the tunnel's current ingress into the route table. Returns
