...
fn render_cloudflared_preview(route: &ProxyRoute) -> String {
    let mut out = String::new();
    out.push_str(&format!(
        "# Managed by klappstuhl.me — Cloudflare Tunnel ingress (part of {CLOUDFLARED_FILE}), route #{} ({})\n",
        route.id, route.subdomain
    ));
...
pub fn render_cloudflared_config(
    ...
    match tunnel {
        ...
        _ => out.push_str("tunnel: REPLACE_WITH_TUNNEL_ID   # set cloudflared_tunnel in config.json\n"),
    }
    match credentials_file {
        Some(c) if !c.trim().is_empty() => out.push_str(&format!("credentials-file: {}\n", c.trim())),
        _ => out.push_str(
            "credentials-file: /etc/cloudflared/REPLACE.json   # set cloudflared_credentials_file in config.json\n",
        ),
    }
    out.push('\n');
    out.push_str("ingress:\n");
...
mod tests {
    ...
    fn missing_tunnel_emits_placeholder() {
        ...
        assert!(cfg.contains("cloudflared_credentials_file"));
    }
}
