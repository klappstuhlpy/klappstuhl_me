...
fn cell_to_string(row: &tokio_postgres::Row, idx: usize) -> String {
    ...
    let s_get =
        |row: &tokio_postgres::Row, i: usize| -> Option<String> { row.try_get::<_, Option<String>>(i).ok().flatten() };

    if ty == &Type::TEXT || ty == &Type::VARCHAR || ty == &Type::BPCHAR || ty == &Type::NAME || ty == &Type::UNKNOWN
    //|| ty == &Type::CITEXT
    {
