    ...
    state
        ...
        .call(|conn| -> rusqlite::Result<Vec<ProxyRoute>> {
            let mut stmt = conn.prepare_cached(&format!(
                "SELECT {SELECT_COLS} FROM proxy_route ORDER BY subdomain ASC"
            ))?;
            let rows: rusqlite::Result<Vec<ProxyRoute>> =
                stmt.query_map([], ProxyRoute::from_row)?.collect();
            rows
        })
        .await
}
...
pub async fn get_route(state: &AppState, id: i64) -> rusqlite::Result<Option<ProxyRoute>> {
    state
        ...
        .call(move |conn| -> rusqlite::Result<Option<ProxyRoute>> {
            let mut stmt = conn
                .prepare_cached(&format!("SELECT {SELECT_COLS} FROM proxy_route WHERE id = ?"))?;
