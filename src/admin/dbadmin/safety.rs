...
/// True when the SQL appears to be a read-only statement (`SELECT`,
...
pub fn is_safe_query(sql: &str) -> bool {
    ...
    // For WITH …, scan past the CTE list to the first non-WITH keyword.
...
/// Returns the first SQL keyword (letters only) at the start of `sql`,
/// or an empty string if the input doesn't start with an identifier.
...
/// Best-effort comment stripper. Removes `-- line comments` and
/// `/* block comments */`. Doesn't try to be SQL-correct inside string
/// literals — it only matters that we get past leading comments.
...
mod tests {
    ...
    fn allows_basic_selects() {
        ...
        assert!(is_safe_query("  select * from users;"));
    ...
    fn rejects_writes() {
        ...
        assert!(!is_safe_query("TRUNCATE x"));
        assert!(!is_safe_query("GRANT ALL ON x TO postgres"));
    }
