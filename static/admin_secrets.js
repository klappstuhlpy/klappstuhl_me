/* ── Secret-scanner dashboard ──────────────────────────────────
   Polls /admin/secrets/data on filter change + after manual scan.
   Status changes are POSTed to /admin/secrets/:id/status and the
   table is re-fetched.
   ───────────────────────────────────────────────────────────── */
...
async function loadData() {
    const url = `/admin/secrets/data?status=${encodeURIComponent(currentFilter)}`;
...
function renderFindings(rows) {
    ...
    tbody.innerHTML = rows.map(r => {
        const sevCls = r.severity;       // 'critical' | 'high' | 'medium'
...
async function updateStatus(id, status) {
    ...
    const res = await fetch(`/admin/secrets/${id}/status`, {
        method: "POST",
...
document.getElementById("scan-now-btn").addEventListener("click", async () => {
    ...
    try {
        const res = await fetch("/admin/secrets/scan", { method: "POST" });
        const data = await res.json();
        if (!data.started) {
            alert(data.detail);
        } else {
            // Give the background scan ~3s to do something before refresh.
