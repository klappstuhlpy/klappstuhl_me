...
async function loadData() {
    const res = await fetch("/admin/firewall/data");
    if (!res.ok) return;
...
function renderLockouts(rows) {
    ...
    tbody.querySelectorAll("button[data-action='release']").forEach(btn => {
        btn.addEventListener("click", async (ev) => {
            ...
            await fetch(`/admin/firewall/lockout/${id}/release`, { method: "POST" });
            loadData();
        });
    });
}
...
async function handleRuleAction(id, action, rule) {
    if (action === "delete") {
        ...
        const res = await fetch(`/admin/firewall/rule/${id}`, { method: "DELETE" });
        if (res.ok) loadData();
    } else if (action === "toggle") {
        const body = new URLSearchParams({ enabled: rule.enabled ? "false" : "true" });
        const res = await fetch(`/admin/firewall/rule/${id}/toggle`, {
            method: "POST",
...
document.getElementById("rule-form").addEventListener("submit", async (ev) => {
    ...
    const res = await fetch("/admin/firewall/rule", {
        method: "POST",
        headers: { "content-type": "application/x-www-form-urlencoded" },
        body,
    });
...
document.getElementById("lockout-form").addEventListener("submit", async (ev) => {
    ...
    const res = await fetch("/admin/firewall/lockout", {
        method: "POST",
        headers: { "content-type": "application/x-www-form-urlencoded" },
        body,
    });
...
document.getElementById("reapply-btn").addEventListener("click", async () => {
    ...
    btn.disabled = true;
    try {
        const res = await fetch("/admin/firewall/apply", { method: "POST" });
        const data = await res.json();
