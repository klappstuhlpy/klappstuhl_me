...
(function () {
    ...
    async function loadContainers() {
        ...
        try {
            const r = await fetch('/admin/docker/graph');
    ...
    btnCreate.addEventListener('click', async () => {
        ...
        try {
            const r = await fetch('/admin/docker/snapshots', {
                method: 'POST',
    ...
    async function loadSnapshots() {
        ...
        try {
            const r = await fetch('/admin/docker/snapshots/data');
            if (!r.ok) throw new Error(`HTTP ${r.status}`);
    ...
    async function doDelete(id, btn) {
        btn.disabled = true;
        try {
            const r = await fetch(`/admin/docker/snapshots/${id}`, { method: 'DELETE' });
            if (!r.ok) {
    ...
    btnRestoreOk.addEventListener('click', async () => {
        ...
        try {
            const r = await fetch(`/admin/docker/snapshots/${pendingRestoreId}/restore`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
