/* ── Health / uptime dashboard ──────────────────────────────────
   - Polls /admin/health/data every 15s
...
async function loadData() {
    const res = await fetch("/admin/health/data");
    if (!res.ok) return;
...
function renderIncidents(summaries) {
    ...
    fetch("/admin/health/incidents?limit=100").then(r => r.json()).then(rows => {
...
async function handleAction(id, action, target) {
    if (action === "probe") {
        ...
        const res = await fetch(`/admin/health/${id}`, { method: "DELETE" });
        if (res.ok) loadData();
    } else if (action === "edit") {
        openModal(target);
    }
...
document.getElementById("health-form").addEventListener("submit", async (ev) => {
    ...
    const url = id ? `/admin/health/${id}` : "/admin/health";
...
if (window.LiveBus) {
    window.LiveBus.subscribe("health", () => loadData());
    window.LiveBus.subscribe("health.event", () => loadData());
}
